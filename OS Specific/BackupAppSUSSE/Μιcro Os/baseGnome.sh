...
#Cli apps
sudo tukit execute -c zypper -n in oneko neofetch tree distrobox

#filesystem utils 
sudo tukit execute -c zypper -n  in   exfatprogs  fatsort fuse-exfat ntfs-3g ntfsprogs ntfsprogs-extra 
#wanted apps
sudo tukit execute  -c  zypper -n in fish grep nano  libfsapfs   make gcc      sed curl openssl       qemu virt-manager unrar   vncmanager xorg-x11-Xvnc-novnc  gamemoded neofetch firewalld
